...
const genUsername = async (length, Accounts, i = 0) => {
  if (i > Math.pow(26, length))
    throw new Error("can not generate unique username");
  const randomLetters = (length, i = 0, result = "") => {
    if (i === length) return result;
    result += String.fromCharCode(97 + Math.floor(Math.random() * 26));
    return randomLetters(length, i + 1, result);
  };
  const username = randomLetters(length);
  const checkUser = await Accounts.findOne({
    where: {
      username: username,
    },
  });
  if (checkUser) return genUsername(length, Accounts, i + 1);
  return username;
};
...
users.signUp = async (userInfo) => {
  ...
  const username = genUsername(5);
  const dbPassword = uniqueNamesGenerator({
