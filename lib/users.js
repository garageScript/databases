...
users.resetUserPassword = async (token, password) => {
  ...
  const unencryptedToken = Buffer.from(token, 'base64').toString('ascii')
  const account = JSON.parse(unencryptedToken)

  const {userid, userToken} = account 
  const {Accounts} = db.getModels()
  ...
  if (userAccount.tokenExpiration !== userToken) {
