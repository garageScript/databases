...
const sendUserConfirmation = users.sendUserConfirmation
...
describe('Validates user signup', () => {
  ...
  it('should send a user email confirmation', async () => {
    ...
    await sendUserConfirmation(obj)
    expect(email.sendConfirmationEmail.mock.calls.length).toEqual(1)
    expect(email.sendConfirmationEmail.mock.calls[0][0]).toEqual('Batman')
  })
})
