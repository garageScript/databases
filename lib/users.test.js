...
const {signUp, logIn, setDBPassword} = require('./users')
...
const { signUp, logIn, sendPasswordResetEmail } = users
...
describe('set DB Password', () => {
  ...
  it('should throw error if input is invalid', async () => {
    const obj = {}
    return expect(setDBPassword(obj)).rejects.toThrow('invalid input: username and DB password cannot be empty')
  })
